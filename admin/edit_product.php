    ...
    if (empty($name) || $price <= 0) {
        ...
        if (!empty($_FILES['image']['name'])) {
            ...
            if (move_uploaded_file($image_temp, $upload_dir . $image_name)) {
                ...
                $image_name = $product['image']; // Keep existing image on error
            }
        }
        ...
        if (empty($message)) {
            try {
                $stmt = $connexion->prepare('
                    UPDATE products 
                    SET name = ?, description = ?, price = ?, stock = ?, category_id = ?, is_featured = ?, image = ?, updated_at = NOW()
                    WHERE id = ?
                ');
                $stmt->execute([$name, $description, $price, $stock, $category_id, $is_featured, $image_name, $product_id]);
...
<head>
    ...
    <title>DermaShop Admin - Edit Product</title>
                ...
                <form method="POST" action="" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <div class="current-image">
                            <p>Current image:</p>
                            <img src="../img/products/<?php echo $product['image']; ?>" alt="<?php echo htmlspecialchars($product['name']); ?>">
                        ...
                        <small>Upload a new image to replace the current one. Recommended size: 600x600 pixels. Max size: 2MB.</small>
                    </div>
